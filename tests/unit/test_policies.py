...
class LoadBalancingPolicyTest(unittest.TestCase):
    def test_non_implemented(self):
        ...
        host = Host("ip1", SimpleConvictionPolicy, 9042)
...
class DCAwareRoundRobinPolicyTest(unittest.TestCase):
    ...
    def test_no_remote(self):
        hosts = []
        for i in range(4):
            h = Host(i, SimpleConvictionPolicy, 9042)
    ...
    def test_with_remotes(self):
        hosts = [Host(i, SimpleConvictionPolicy, 9042) for i in range(4)]
        for h in hosts[:2]:
            h.set_location_info("dc1", "rack1")
        for h in hosts[2:]:
    ...
    def test_get_distance(self):
        ...
        remote_host = Host("ip2", SimpleConvictionPolicy, 9042)
        ...
        second_remote_host = Host("ip3", SimpleConvictionPolicy, 9042)
    ...
    def test_status_updates(self):
        ...
        new_local_host = Host(4, SimpleConvictionPolicy, 9042)
        new_local_host.set_location_info("dc1", "rack1")
        policy.on_up(new_local_host)

        new_remote_host = Host(5, SimpleConvictionPolicy, 9042)
    ...
    def test_modification_during_generation(self):
        ...
        new_host = Host(4, SimpleConvictionPolicy, 9042)
        ...
        another_host = Host(5, SimpleConvictionPolicy, 9042)
    ...
    def test_default_dc(self):
        host_local = Host(1, SimpleConvictionPolicy, 9042, 'local')
        host_remote = Host(2, SimpleConvictionPolicy, 9042, 'remote')
        host_none = Host(1, SimpleConvictionPolicy, 9042)

        # contact point is '1'
        cluster = Mock(contact_points_resolved=[(1, 9042)])
...
class TokenAwarePolicyTest(unittest.TestCase):
    ...
    def test_wrap_round_robin(self):
        ...
        hosts = [Host(str(i), SimpleConvictionPolicy, 9042) for i in range(4)]
        for host in hosts:
            host.set_up()
...
class WhiteListRoundRobinPolicyTest(unittest.TestCase):
    ...
    def test_hosts_with_hostname(self):
        ...
        host = Host("127.0.0.1", SimpleConvictionPolicy, 9042)
...
class HostFilterPolicyDistanceTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ignored_host = Host(inet_address='ignoreme', conviction_policy_factory=Mock(), port=9042)
        self.accepted_host = Host(inet_address='acceptme', conviction_policy_factory=Mock(), port=9042)
...
class HostFilterPolicyQueryPlanTest(unittest.TestCase):
    ...
    def test_wrap_token_aware(self):
        cluster = Mock(spec=Cluster)
        hosts = [Host("127.0.0.{}".format(i), SimpleConvictionPolicy, 9042) for i in range(1, 6)]
        for host in hosts:
            host.set_up()
        ...
        self.assertEqual(query_plan[0], Host("127.0.0.2", SimpleConvictionPolicy, 9042))
        self.assertEqual(set(query_plan[1:]),{Host("127.0.0.3", SimpleConvictionPolicy, 9042),
                                              Host("127.0.0.5", SimpleConvictionPolicy, 9042)})
    ...
    def test_create_whitelist(self):
        ...
        self.assertEqual(set(query_plan), {Host("127.0.0.1", SimpleConvictionPolicy, 9042),
                                           Host("127.0.0.4", SimpleConvictionPolicy, 9042)})
