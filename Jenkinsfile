...
DEFAULT_RUNTIME = ['3.9.16', '3.10.11', '3.11.3', '3.12.0']
...
pipeline {
  ...
  triggers {
    parameterizedCron(branchPatternCron().matcher(env.BRANCH_NAME).matches() ? """
      ...
      # These schedules will run with and without Cython enabled for Python 3.9.16 and 3.12.0
      H 4 * * 1-5 %CI_SCHEDULE=WEEKNIGHTS;EVENT_LOOP=LIBEV;CI_SCHEDULE_PYTHON_VERSION=3.9.16 3.12.0;CI_SCHEDULE_SERVER_VERSION=3.11 4.0 5.0 dse-5.1.35 dse-6.8.30 dse-6.9.0 hcd-1.0.0
    """ : "")
  }
